...
/** TODO make this a normal function that returns an array of stuff? */
...
//TODO: add a tooltip
   ...
   const bottomLine = <span className="small">
      {maybeBold(gradeFormat == "rank", <a href="#" onClick={(event) => { event.preventDefault(); setConfig(`rank:${tierStrTmp}`); }}>Ranks</a>)}&nbsp;
      | {maybeBold(gradeFormat == "pct", <a href="#" onClick={(event) => { event.preventDefault(); setConfig(`pct:${tierStrTmp}`); }}>Pctiles</a>)}
   </span>

   const teamPercentiles = tierToUse ? GradeUtils.buildTeamPercentiles(tierToUse, team, gradeFormat == "rank")  : {};

   // Special field formatting:
   (teamPercentiles as any).off_title = gradeFormat == "pct" ? "Off %iles" : "Off Eq Ranks";
   (teamPercentiles as any).def_title = gradeFormat == "pct" ? "Def %iles" : "Def Eq Ranks";
//TODO: add a tooltip
