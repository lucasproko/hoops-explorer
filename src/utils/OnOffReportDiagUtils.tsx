...
export class OnOffReportDiagUtils {
  ...
  //TODO: for offense and defense, show best/worst 5 lineups in terms of contrib
  // and ditto for peers
  // Then also have buttons to expand and sort full lineup view
    ...
    return _.chain(player?.replacement?.myLineups)
      .map((lineup: any) => {
        ...
        return {
          ...
          contrib: {
            offTotalPoss: (lineup?.off_poss?.value || 0)/offTotalPoss,
            offAdjEff: offContrib,
            defTotalPoss: (lineup?.off_poss?.value || 0)/offTotalPoss,
            defAdjEff: defContrib
          },
  ...
  static getRepOnOffDiags(
    ...
    repOnOffDiagMode: number,
    showHelp: boolean
  ) {
    return _.flatten([
      [ OnOffReportDiagUtils.getTitle(player, showHelp) ],
      _.chain(lineupsPlusDiags)
        .sortBy([(lineupPlusDiag) => -lineupPlusDiag.lineup?.off_poss.value])
        .take(repOnOffDiagMode).flatMap((lineupPlusDiag: any) => {
          ...
          const lineupSummary =
            _.chain(player?.replacement?.lineupUsage || {})
              .pick(lineupPlusDiag.lineup.offLineupKeys || [])
              .toPairs()
          ...
          const offContrib = lineupPlusDiag.contrib.offAdjEff;
          const defContrib = lineupPlusDiag.contrib.defAdjEff;
