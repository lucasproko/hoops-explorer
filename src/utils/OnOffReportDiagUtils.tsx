...
export class OnOffReportDiagUtils {
  ...
  /** Based on the diag info, build the stats for a list of peers */
  static buildPeerStatsForPlayer(
    player: Record<string, any>,
    lineupsPlusDiags: Array<Record<string, any>> // generated by getRepOnOffDiagInfo
  ) {
    return _.chain(lineupsPlusDiags || []).transform((acc, v) => {
      _.chain(v.peers || []).toPairs().forEach((peerKv) => {
        const peerId = peerKv[0];
        const peerObj = peerKv[1];
        const currPeer = acc[peerId];
        if (!currPeer) {
          acc[peerId] = _.clone(peerObj);
        } else {
          currPeer.poss += peerObj.poss;
          currPeer.off_adj_ppp += peerObj.off_adj_ppp*peerObj.poss;
          currPeer.def_adj_ppp += peerObj.def_adj_ppp*peerObj.poss;
        }
      }).value();
    }, {} as Record<string, any>).mapValues((peerObj) => {
      return {
        poss: peerObj.poss,
        off_adj_ppp: peerObj.off_adj_ppp/(peerObj.poss || 1),
        def_adj_ppp: peerObj.def_adj_ppp/(peerObj.poss || 1),
      };
    }).value();
  }
