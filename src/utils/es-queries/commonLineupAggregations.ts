...
const calculateAdjEff = function (offOrDef: "off" | "def") {
  return `
  def hca = 0.0;
...
const commonShotAggs = function (
  ...
  return {
    ...commonMiscAggs(
      srcPrefix,
      dstPrefix,
      `fg_${altShotType || shotType}.attempts`,
      `${shotType}_attempts`,
      suffix
    ),
    ...commonMiscAggs(
...
/** shotType is 2p, 3p, 2prim, 2pmid */
const commonAverageShotAggs = function (
  dstPrefix: "off" | "def",
  shotType: string
) {
  return {
...
const commonAssistInfo = function (
  srcPrefix: string,
  dstPrefix: "off" | "def",
  shotType: string
) {
...
export const commonAggregations = function (
  ...
  const typeSuffixFromPrefix = (typePrefix: "" | "scramble_" | "trans_") => {
    switch (typePrefix) {
      case "":
        return ".total";
      case "scramble_":
  ...
  const typePrefixes = ["", "scramble_", "trans_"] as (
    | ""
    | "scramble_"
    | "trans_"
  )[];
  ...
  return {
      ...
      (acc, typePrefix) => {
        return _.merge(acc, {
          ...commonMiscAggs(
            ...
            "to",
            typePrefix + "to",
            typeSuffixFromPrefix(typePrefix)
          ),
          ...commonMiscAggs(
            srcPrefix,
            dstPrefix,
      ...
      (acc, typePrefix) => {
        return _.merge(acc, {
          ...
          ...commonAverageAggs(
            dstPrefix,
            typePrefix + "2primr",
            typePrefix + "2prim_attempts",
            typePrefix + "fga"
          ),
          ...commonAverageAggs(
            dstPrefix,
            typePrefix + "2pmidr",
            typePrefix + "2pmid_attempts",
            typePrefix + "fga"
          ),
          ...commonAverageAggs(
            dstPrefix,
            typePrefix + "3pr",
            typePrefix + "3p_attempts",
            typePrefix + "fga"
          ),
    ...
    ..._.transform(
      ...
      (acc, typePrefix) => {
        return _.merge(acc, {
          ...
          [`total_${dstPrefix}_${typePrefix}poss`]: {
            //fgm + (approx-unrebounded) fgM + 0.475*fta + to
            bucket_script: {
              buckets_path: {
    ...
    // Dumb rename:
    [`${dstPrefix}_poss`]: {
      //TODO fix this, there's a mess of total prefixes
      bucket_script: {
        buckets_path: {
    ...
    [`${dstPrefix}_orb`]: {
      bucket_script: {
        ...
        script:
          "(params.var_orb > 0) ? 1.0*params.var_orb/(params.var_orb + params.var_drb) : 0.0",
      },
    },
    // eFG:
    ..._.transform(
...
const shotInfoAggregations = function () {
  return _.chain(LuckUtils.lineupShotInfoFields)
    .flatMap((field) => {
      return [
        [
          `shot_info_${field}`,
...
export const commonLineupAggregations = function (
  publicEfficiency: any,
  lookup: any,
  avgEff: number,
  hca: number,
  lineupAgg: Boolean = false
) {
  return {
    // Derived
    ...commonAggregations(
