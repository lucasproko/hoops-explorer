...
export class OverrideUtils {
  ...
  static readonly clearTeamManualOrLuckOverrides = (mutableTeamStats: TeamStatSet) => {
    _.keys(OverrideUtils.getOverridableStats(ParamPrefixes.player)).concat([
      "off_ppp", "off_adj_ppp", "off_net", "off_raw_net", "off_efg", //(derived values for manual/luck)
  ...
  static readonly applyPlayerOverridesToTeam = (
    ...
    const adjustStat = (mutableStat: Statistic | undefined, delta: number, over: ManualOverride) => {
    ...
    overrides.filter(o => o.use).flatMap(override => {
      ...
      if (player) {
        ...
        if (weightField && mutableTeamStats[weightField] && player[weightField]) {
          ...
          adjustStat(mutableTeamStats[override.statName], weightedDelta*deltaStat, override);
        }
        ...
        adjustStat(mutableTeamStats.off_ppp, offPppDelta, override);
        ...
        adjustStat(mutableTeamStats.off_adj_ppp, adjustedOffPppDelta, override);
        ...
        if (_.endsWith(weightField, "_attempts")) { //(ie adjusted a shot, not TO% / FT%)
          ...
          adjustStat(mutableTeamStats["off_efg"], weightedEfgDelta*eFgDeltaStat, override);
        }

        // Adjust the adjusted and raw net margins:

        adjustStat(mutableTeamStats.off_raw_net, offPppDelta, override);
        adjustStat(mutableTeamStats.off_net, adjustedOffPppDelta, override);
      }
    });
