...
import { IndivStatSet, Statistic, TeamStatSet, LineupStatSet, OnOffBaselineEnum } from '../StatModels';
...
export class OverrideUtils {
  ...
  static readonly clearTeamManualOrLuckOverrides = (mutableTeamStats: TeamStatSet) => {
    _.keys(OverrideUtils.getOverridableStats(ParamPrefixes.player)).concat([
      "off_ppp", "off_adj_ppp", "off_net", "off_raw_net", "off_efg", "off_2p", //(derived values for manual/luck)
      "def_ppp", "def_adj_ppp", "def_3p", "def_efg" //(derived values for luck only)
    ]).forEach(key => {
  ...
  static readonly filterManualOverrides = (queryKey: OnOffBaselineEnum, overrides?: ManualOverride[]) => {
    return (overrides || []).filter(o => o.use).flatMap(override => {
  ...
  static readonly applyPlayerOverridesToTeam = (
    queryKey: OnOffBaselineEnum, overrides: ManualOverride[],
    playerStats: Record<string, IndivStatSet>, mutableTeamStats: TeamStatSet,
    averageEff: number, adjustForLuck: Boolean
  ) => {
    // Apply overrides

    const adjustStat = (mutableStat: Statistic | undefined, delta: number, over: ManualOverride, notIfLuckAdj: Boolean = false) => {
      if (mutableStat && _.isNumber(mutableStat.value)) {
        const existingVal = mutableStat.value;
        if (!notIfLuckAdj) { //(if luck adjusting, the aggregate params will be written by LuckUtils)
        ...
        const formattedDelta = isPct ? (delta*100).toFixed(1) : delta.toFixed(1);
        const newOverrideStr = 
          `Adjusted by [${formattedDelta}] for [${over.rowId}] override of [${over.statName}] to [${100*over.newVal}]%`;
    ...
    OverrideUtils.filterManualOverrides(queryKey, overrides).forEach(override => {
      const player = playerStats[override.rowId];

      if (player) {
        const deltaStat = 
          (player[override.statName]?.value || 0) - (player[override.statName]?.old_value || 0);

        const weightField = OverrideUtils.overrideFieldToWeightField[override.statName];
        if (weightField && mutableTeamStats[weightField] && player[weightField]) {
          const weightedDelta = (player[weightField].value || 0)/(mutableTeamStats[weightField].value || 1);
          adjustStat(mutableTeamStats[override.statName], weightedDelta*deltaStat, override, false);

          // If adjusting the 2P shooting then adjust aggregate 2P% (used in Luck calcs)
          if (("off_2pmid" == override.statName) || ("off_2prim" == override.statName)) {            
            const twoPtFgWeight = "total_off_2p_attempts";
            const weightedEfgDelta = (player[weightField].value || 0)/(mutableTeamStats[twoPtFgWeight]?.value || 1);
            adjustStat(mutableTeamStats["off_2p"], weightedEfgDelta*deltaStat, override, false);
          }
        }
        // ORtg -> unadjusted efficiency
        const deltaORtg = (player.off_rtg?.value || averageEff) - (player.off_rtg?.old_value || averageEff);
        ...
        adjustStat(mutableTeamStats.off_ppp, offPppDelta, override, adjustForLuck);
        ...
        adjustStat(mutableTeamStats.off_adj_ppp, adjustedOffPppDelta, override, adjustForLuck);

        // eFG
        if (_.endsWith(weightField, "_attempts")) { //(ie adjusted a shot, not TO% / FT%)
          const eFgWeight = "total_off_fga";
          const eFgDeltaStat = 
            (player.off_efg?.value || 0) - (player.off_efg?.old_value || 0);
          const weightedEfgDelta = (player[eFgWeight]?.value || 0)/(mutableTeamStats[eFgWeight]?.value || 1);
          adjustStat(mutableTeamStats["off_efg"], weightedEfgDelta*eFgDeltaStat, override, adjustForLuck);
        }

        // Adjust the adjusted and raw net margins:

        adjustStat(mutableTeamStats.off_raw_net, offPppDelta, override, adjustForLuck);
        adjustStat(mutableTeamStats.off_net, adjustedOffPppDelta, override, adjustForLuck);
      }
    });
