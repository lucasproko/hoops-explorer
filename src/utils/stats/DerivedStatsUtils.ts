import { TeamStatSet, PureStatSet } from '../StatModels';
...
export class DerivedStatsUtils {
    ...
    */
    static readonly injectScrambleStats = (stat: TeamStatSet, offDef: "off" | "def", toMutate: PureStatSet) => {
    ...
    */
     static readonly injectTransitionStats = (stat: TeamStatSet, offDef: "off" | "def", toMutate: PureStatSet) => {
      const totalPoss = stat[`total_${offDef}_poss`]?.value || 1;
    ...
    /** Injects some 4-8 factor stats for transition and scrambles
     * Fields injected (off/def unless specified): _${playType}_to, _${playType}_ftr, _${playType}_3pr, _${playType}_3p, _${playType}_2p
    */
   static readonly injectStatBreakdowns = (stat: TeamStatSet, offDef: "off" | "def", playType: "scramble" | "trans", toMutate: PureStatSet) => {
   ...
   */
   static readonly injectPaceStats = (stat: TeamStatSet, toMutate: PureStatSet) => {
      ...
      if (totalTime) {
         toMutate[`tempo`] = { value: 0.5*(totalOffPoss + totalDefPoss) / (totalTime/40) };
      };
      return toMutate;
   }

   /** Injects all the derived stats in one go */
   static readonly injectDerivedStats = (stat: TeamStatSet, toMutate: PureStatSet) => {
      ...
      DerivedStatsUtils.injectPaceStats(stat, toMutate);
      return toMutate;
   }
