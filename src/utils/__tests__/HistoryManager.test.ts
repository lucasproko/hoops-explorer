...
describe("HistoryManager", () => {
  ...
  test("HistoryManager - gameFilterSummary", () => {
    ...
    const game2: GameFilterParams = {
      ...
      onQueryFilters: "Home", offQueryFilters: "Conf",
    ...
    expect(HistoryManager.filterSummary("game-", game2)).toBe(
      `On/Off: 2019/20 test (M) [1:150] [!garbage] [+Conf]: on:'testOn'/[+Home], off:'testOff'/[+Conf], base:'testBase', luck:[baseline], [overrides], team:[on-off-luck,show-roster,show-play-types,show-extra-info]`
    );
    const game3: GameFilterParams = {
      team: "test",
      manual: [{ newVal: 1, rowId: "test", use: true, statName: "test" }],
      showTeamPlayTypes: true,
      showExtraInfo: true,
      ...
      onQuery: "testOn", baseQuery: "testBase",
      onQueryFilters: "Home", offQueryFilters: "Conf",
      offQuery: "testOff", autoOffQuery: true,
      luck: { base: "baseline" },
      onOffLuck: true,
      filterGarbage: true, queryFilters: "Conf"
    };
    expect(HistoryManager.filterSummary("game-", game3)).toBe(
      `On/Off: 2019/20 test (M) [1:150] [!garbage] [+Conf]: on:'testOn'/[+Home], auto-off, base:'testBase', luck:[baseline], [overrides], team:[on-off-luck,show-roster,show-games,show-play-types,show-extra-info]`
    );
  });
  ...
  test("HistoryManager - teamReportFilterSummary", () => {
    ...
    expect(HistoryManager.teamReportFilterSummary(_.assign(_.clone(report2), { incRapm: true, rapmDiagMode: "base", rapmPriorMode: 0.5, rapmRegressMode: "0.2" }))).toBe(
      `On/Off Report: 2018/19 team3 (W) [10:370]: query:'test ""' (sort:test-sort, filter:'Test"Filter', show:[rapm:prior=0.5:regress=0.2,rapm:diag])`
    );
