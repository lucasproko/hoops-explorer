...
import CustomMatchupFilter from "../components/CustomMatchupFilter";
...
const CustomMatchupAnalyzerPage: NextPage<{}> = () => {
  useEffect(() => {
    // Set up GA
  ...
  function getRootUrl(params: MatchupFilterParams) {
    return UrlRouting.getCustomMatchupUrl(params);
  }
  ...
  const lineupStintTable = restrictiveMode
    ? undefined
    : React.useMemo(() => {
        return (
          <GenericCollapsibleCard
  ...
  const playStyleChart =
    !restrictiveMode ||
    _.includes(startingMatchupFilterParams.customDisplayMode, "playTypes")
      ? React.useMemo(() => {
                    ...
                    <Col xs={12}>
                      {_.isEmpty(divisionStatsCache) ? (
                        ...
                        PlayTypeDiagUtils.buildTeamStyleBreakdown(
                          ...
                          {
                            ...dataEvent.teamStatsA,
                            on: dataEvent.teamStatsA.off,
                            baseline: dataEvent.teamStatsA.on,
                          },
                          avgEfficiency,
                          divisionStatsCache,
                          showHelp,
                    ...
                    <Col xs={12}>
                      {_.isEmpty(divisionStatsCache) ? (
                        ...
                        PlayTypeDiagUtils.buildTeamStyleBreakdown(
                          ...
                          {
                            ...dataEvent.teamStatsB,
                            on: dataEvent.teamStatsB.off,
                            baseline: dataEvent.teamStatsB.on,
                          },
                          avgEfficiency,
                          divisionStatsCache,
                          showHelp,
  ...
  const shotChart = restrictiveMode
    ? undefined
    : React.useMemo(() => {
        return (
          <GenericCollapsibleCard
  ...
  const teamStatsTableTeam =
    restrictiveMode &&
    _.includes(startingMatchupFilterParams.customDisplayMode, "teamStats")
      ? React.useMemo(() => {
          ...
          return (
            <GenericCollapsibleCard
              minimizeMargin={true}
              title={`Team Analysis [${matchupFilterParams.team || "Unknown"}]`}
              helpLink={maybeShowDocs()}
            >
              <TeamStatsTable
                gameFilterParams={matchupFilterParams}
                dataEvent={{
                  teamStats: {
                    ...dataEvent.teamStatsA,
                    onOffMode: false,
                  },
                  rosterStats: dataEvent.rosterStatsA,
                  shotStats: {
                    on: { off: {}, def: {} },
                    off: { off: {}, def: {} },
                    baseline: { off: {}, def: {} },
            ...
            >
              <TeamStatsTable
                ...
                dataEvent={{
                  ...
                  rosterStats: dataEvent.rosterStatsB,
                  shotStats: {
                    on: { off: {}, def: {} },
                    off: { off: {}, def: {} },
                    baseline: { off: {}, def: {} },
  ...
  //(this code fools SSR, somehow! Otherwise the display gets crazy mixed up)
  if (!allParams)
    return (
      <Container>
        <Row ref={topRef}>
          <Col xs={12} className="text-center">
            <h3>CBB Match-up Analysis Tool - Custom Display Mode </h3>
          </Col>
        </Row>
        <Row>
          <HeaderBar
  ...
  return restrictiveMode ? (
    <Container>
      ...
      {teamStatsTableTeam ? <Row>{teamStatsTableTeam}</Row> : undefined}
      {teamStatsTableOppo ? <Row>{teamStatsTableOppo}</Row> : undefined}
      <Footer
...
export default CustomMatchupAnalyzerPage;
