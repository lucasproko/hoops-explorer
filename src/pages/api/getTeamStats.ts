    ...
    if (req.headers?.["accept-encoding"]?.includes("gzip")) {
      try {
        ...
        const resp = await fetch(
          `https://storage.googleapis.com/${process.env.LEADERBOARD_BUCKET}/team_stats_${urlInFix}${maybePosInfix}all_${parsed.gender}_${parsed.year}_${parsed.tier}.json.gz?cacheId=${cacheId}`,
          {
            method: "get",
          }
        );
