...
function marshallRequest(
  ...
  const needEff =
    Number(params.minRank || "0") > 0 ||
    Number(params.maxRank || "400") < 400 ||
    (params.queryFilters || "").indexOf("Conf") >= 0 ||
    params.baseQuery ||
    params.onQuery ||
    params.offQuery; //(need these in case we're using eg vs_rank)
  const body =
    [
      JSON.stringify({ index: index }),
      JSON.stringify(gameInfoQuery(params, currentJsonEpoch)),
