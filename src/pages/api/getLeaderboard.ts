...
if (pxIsDebug) {
  console.log(
    `Running locally (getLeaderboard): use node v18+ constructs for streaming`
  );
}
    ...
    if (req.headers?.["accept-encoding"]?.includes("gzip")) {
      try {
        ...
        const resp = await fetch(
          `https://storage.googleapis.com/${process.env.LEADERBOARD_BUCKET}/${parsed.src}_${parsed.oppo}_${parsed.gender}_${parsed.year}_${parsed.tier}.json.gz?cacheId=${cacheId}`,
          {
            method: "get",
          }
        );
